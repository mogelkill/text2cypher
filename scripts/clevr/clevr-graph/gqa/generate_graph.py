...
from .args import *
from .types import EdgeSpec, GraphSpec, LineSpec, NodeSpec
...
class GraphGenerator:
    ...
    def gen_a(self, Clz, prop_dict):
        return Clz({k: random.choice(prop_dict[k]) for k in prop_dict.keys()})
    ...
    def gen_stations(self):
        ...
        def find_nearby_stations():
            ...
            return frozenset([frozenset([features[i] for i in group]) for group in ind if len(group) > 1])
    ...
    def gen_graph_spec(self):
        ...
        self.graph_spec = GraphSpec(nodes, edges, lines)
