...
from gqa.types import EdgeSpec, LineSpec, NodeSpec
...
class GqlBuilder:
    def __init__(self, fp: Dict[str, Any]):
        ...
        self.op_inputs = {"Station": self.node_input_argument, "Line": self.line_input_argument}
    ...
    def get_lines(self, a: Var):
        ...
        self.current_where.append(f"{var2}.line_id = {var}.id")
        var = self.get_var()
    ...
    def has_intersection(self, a: Var, b: Var):
        ...
        vars = self.get_all_vars()
        var = self.get_var()
        self._stack.append(f"WITH {vars}, length(apoc.coll.intersection(collect({a}), collect({b}))) > 0 AS {var}")
        return var
    ...
    def _recurse(self, fp):
        ...
        assert len(fp) == 1, f"Functional program can only contain a single function at a time, found {fp}"
